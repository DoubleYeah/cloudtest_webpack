  ...
  export default {
    ...
    methods: {
      init() {
        ...
        this.content && $(this.content).each(function (i, item) {
           add_item(item);
        });
      ...
      setEnabled(itemid) {
        $("> .l-menu-item[menuitemid=" + itemid + "]", this.menu.items).removeClass("l-menu-item-disable");
      },
      setMenuText(itemid, text) {
        $("> .l-menu-item[menuitemid=" + itemid + "] >.l-menu-item-text:first", this.menu.items).html(text);
      },
      setDisabled(itemid) {
        $("> .l-menu-item[menuitemid=" + itemid + "]", this.menu.items).addClass("l-menu-item-disable");
      },
      isEnable(itemid) {
        return !$("> .l-menu-item[menuitemid=" + itemid + "]", this.menu.items).hasClass("l-menu-item-disable");
      },
      getItemCount() {
      ...
      addItem(item, menu) {
        ...
        item.img && ditem.prepend(
          '<div class="l-menu-item-icon"><img style="width:16px;height:16px;margin:2px;" src="' + item.img +
          '" /></div>');
        ...
        ditem.hover(function () {
          ...
          if (item.children) {
            ...
            if (this.menus[ligeruimenutemid]) {
              this.show({
                top: itemtop,
                left: $(this).offset().left + $(this).width() - 5
              }, this.menus[ligeruimenutemid]);
      ...
      hideAllSubMenu(menu) {
        ...
        $("> .l-menu-item", menu.items).each(function () {
          if ($("> .l-menu-item-arrow", this).length > 0) {
            var ligeruimenutemid = $(this).attr("ligeruimenutemid");
            if (!ligeruimenutemid) return;
            this.menus[ligeruimenutemid] && this.hide(this.menus[ligeruimenutemid]);
          }
        });
      ...
      createMenu(parentMenuItemID) {
        ...
        if (parentMenuItemID)
          this.menus[parentMenuItemID] = menu;
        else
          this.menus[0] = menu;
        return menu;
      }
